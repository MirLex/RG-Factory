...
joe2 = Customer.new('Joe Smith', '123 Maple, Anytown NC', 12_345)
john = FactoryClass.new('John Smith', '123 Maple, Anytown NC', 12_345)
john2 = FactoryClass.new('Smith John', '123 Maple, Anytown NC', 12_345)
...
meth_with_val = {
  each:       proc { |x| p x.class.to_s },
  each_pair:  proc { |name, value| p("#{name} => #{value}") },
  select:     proc { |val| p val if val.is_a?(Integer) },
  values_at:  1,
  eql?:       john2
}
